	...
	public ActionResult doAction(IAgileSession session, INode arg1, IDataObject dataObject) {
		...
		try {
			...
			if (eCR != null) {
				...
				countOfEmptyAttrs = GenericUtilities.getCountOfIAAttributes(awfMessagesList.get("NULL").toString(),
				...
				if (countOfEmptyAttrs > 0) {
					actionResult = new ActionResult(ActionResult.EXCEPTION,
							new Exception(awfMessagesList.get("VALUES_NOT_FILLED1").toString() + " " + eCR.toString()
									+ "." + awfMessagesList.get("VALUES_NOT_FILLED2").toString()));
				} else {
					...
					if (awf != null) {
						...
						if (relationshipTab != null) {
							...
							result = awf.toString() + " " + awfMessagesList.get("OBJ_CREATED_ADDED_RELTAB").toString()
									+ " " + eCR.toString();
